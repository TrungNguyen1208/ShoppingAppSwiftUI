...
open class DeviceGuru {
    ...
    /// This method returns Is it equal to or smaller than the iPhone SE or not?
    ///
    public func isUnderLineIphoneSE() -> Bool {
        guard let hardwareNumber = self.hardwareNumber() else {
            return false
        }
        let VERSION_IPHONE_SE:Double = 8.4
        
        if Double(hardwareNumber) <= VERSION_IPHONE_SE {
            return true
        } else {
            return false
        }
